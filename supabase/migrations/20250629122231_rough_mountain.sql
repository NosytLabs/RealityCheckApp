/*
  # Offline Features Migration

  1. New Tables
    - `offline_sessions` - Track user offline/focus sessions
    - `blocked_apps` - Apps to block during sessions
    - `offline_challenges` - Predefined challenges for users
    - `user_offline_challenges` - User progress on challenges
  ...
  4. Functions
    - `update_user_stats_after_offline_session()` - Update stats when session completes
    - `setup_default_blocked_apps()` - Initialize default blocked apps for users
*/
...
BEGIN
  ...
  IF NEW.is_completed = true AND (OLD.is_completed IS NULL OR OLD.is_completed = false) THEN
    ...
    UPDATE user_stats SET 
      total_offline_minutes = total_offline_minutes + COALESCE(NEW.duration_minutes, 0),
      offline_session_count = offline_session_count + 1,
      last_offline_session_date = session_date,
      current_offline_streak = consecutive_days,
      longest_offline_streak = GREATEST(longest_offline_streak, consecutive_days),
      updated_at = now()
    WHERE user_id = NEW.user_id;
