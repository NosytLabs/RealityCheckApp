/*
  # Offline Sessions and Challenges Schema
  ...
  1. New Tables
    - `offline_sessions`
      ...
      - `notes` (text, nullable)
      - `apps_blocked` (text array, nullable)
    ...
    - `blocked_apps`
      ...
      - `created_at` (timestamptz, default now)
      - `updated_at` (timestamptz, default now)
      - Unique constraint on (user_id, app_identifier)
    ...
    - `offline_challenges`
      ...
      - `challenge_type` (text, check constraint)
    ...
    - `user_offline_challenges`
      - `id` (uuid, primary key)
      - `user_id` (uuid, foreign key to profiles)
      - `challenge_id` (uuid, foreign key to offline_challenges)
  ...
  2. User Stats Extensions
    - Add offline tracking columns to user_stats table
    - `total_offline_minutes` (integer, default 0)
    - `last_offline_session_date` (date, nullable)
    - `offline_session_count` (integer, default 0)
    - `current_offline_streak` (integer, default 0)
    - `longest_offline_streak` (integer, default 0)
  ...
  4. Functions and Triggers
    - Function to update user stats after offline session completion
    - Triggers for updated_at columns
    - Function to setup default blocked apps for new users

  5. Default Data
    - Insert default offline challenges
    - Setup function for default blocked apps
*/
...
BEGIN
  ...
  IF NEW.is_completed = true AND (OLD.is_completed IS NULL OR OLD.is_completed = false) THEN
    ...
    -- Update user stats using explicit constraint name
    ...
    ON CONFLICT ON CONSTRAINT user_stats_user_id_key
    DO UPDATE SET 
...
-- Insert default offline challenges using individual INSERT statements to avoid conflicts
DO $$
BEGIN
  -- Digital Detox Starter
  IF NOT EXISTS (SELECT 1 FROM offline_challenges WHERE title = 'Digital Detox Starter') THEN
    INSERT INTO offline_challenges (title, description, duration_minutes, points_reward, difficulty, challenge_type, requirements) 
    VALUES ('Digital Detox Starter', 'Stay offline for 30 minutes', 30, 25, 'easy', 'duration', '{"min_duration": 30}');
  END IF;

  -- Focus Hour
  IF NOT EXISTS (SELECT 1 FROM offline_challenges WHERE title = 'Focus Hour') THEN
    INSERT INTO offline_challenges (title, description, duration_minutes, points_reward, difficulty, challenge_type, requirements) 
    VALUES ('Focus Hour', 'Complete a 1-hour offline session', 60, 50, 'medium', 'duration', '{"min_duration": 60}');
  END IF;

  -- Deep Work Session
  IF NOT EXISTS (SELECT 1 FROM offline_challenges WHERE title = 'Deep Work Session') THEN
    INSERT INTO offline_challenges (title, description, duration_minutes, points_reward, difficulty, challenge_type, requirements) 
    VALUES ('Deep Work Session', 'Stay offline for 2 hours straight', 120, 100, 'hard', 'duration', '{"min_duration": 120}');
  END IF;

  -- Daily Disconnect
  IF NOT EXISTS (SELECT 1 FROM offline_challenges WHERE title = 'Daily Disconnect') THEN
    INSERT INTO offline_challenges (title, description, duration_minutes, points_reward, difficulty, challenge_type, requirements) 
    VALUES ('Daily Disconnect', 'Complete 3 offline sessions in one day', 30, 75, 'medium', 'frequency', '{"sessions_per_day": 3}');
  END IF;

  -- Offline Warrior
  IF NOT EXISTS (SELECT 1 FROM offline_challenges WHERE title = 'Offline Warrior') THEN
    INSERT INTO offline_challenges (title, description, duration_minutes, points_reward, difficulty, challenge_type, requirements) 
    VALUES ('Offline Warrior', 'Maintain a 7-day offline streak', 30, 200, 'hard', 'streak', '{"consecutive_days": 7}');
  END IF;

  -- Mindful Morning
  IF NOT EXISTS (SELECT 1 FROM offline_challenges WHERE title = 'Mindful Morning') THEN
    INSERT INTO offline_challenges (title, description, duration_minutes, points_reward, difficulty, challenge_type, requirements) 
    VALUES ('Mindful Morning', 'Start your day with a 45-minute offline session', 45, 60, 'medium', 'duration', '{"min_duration": 45, "time_window": "morning"}');
  END IF;

  -- Evening Wind-down
  IF NOT EXISTS (SELECT 1 FROM offline_challenges WHERE title = 'Evening Wind-down') THEN
    INSERT INTO offline_challenges (title, description, duration_minutes, points_reward, difficulty, challenge_type, requirements) 
    VALUES ('Evening Wind-down', 'End your day with a 30-minute offline session', 30, 40, 'easy', 'duration', '{"min_duration": 30, "time_window": "evening"}');
  END IF;
END $$;
