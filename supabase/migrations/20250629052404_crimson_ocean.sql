...
This migration adds comprehensive offline tracking functionality to the RealityCheck app.

## New Tables
1. **offline_sessions** - Track user offline sessions with duration and completion status
2. **blocked_apps** - Manage apps that are blocked during offline/focus sessions  
3. **offline_challenges** - Predefined challenges for users to complete
4. **user_offline_challenges** - Track user progress on challenges

## Schema Updates
- Added offline tracking columns to user_stats table
...
-- Insert default offline challenges (fixed JSON syntax)
